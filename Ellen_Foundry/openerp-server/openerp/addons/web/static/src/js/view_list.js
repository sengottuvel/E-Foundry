...
//Added by sujith//
        var columnssss = ["stock_date",                                                                "sno","product_name","uom","trans_type","ref_type","dc_no","from_to","qty","price_unit","amount","cl_stock","stock_rate","stock_value"]; 
        // Condition added inorder to disable the sortable feature in the one2many field
        for (i = 0; i < columnssss.length; i++) {
                if (col_name == columnssss[i]){
                        return false;
                }
        }
//Added by sujith//         
    ...
    do_delete: function (ids) {
        var self = this;
            swal({
                ...
                if (isConfirm) {
                    var del = $.when(self.dataset.unlink(ids)).done(function (xhr, status, error) {
