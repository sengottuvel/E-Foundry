...
instance.web.FormView = instance.web.View.extend(instance.web.form.FieldManagerMixin, {
    ...
    load_form: function(data) {
		...
		/***** Start Ticket 2984 */
		if($(this.$el.find('.disable_buttons')).hasClass("disable_edit"))
			$('.oe_form_button_edit').hide();
		
		if($(this.$el.find('.disable_buttons')).hasClass("disable_create"))
			$('.oe_form_button_create').hide();
		
		if($(this.$el.find('.disable_buttons')).hasClass("disable_save")){
			$('.oe_form_button_save').hide();
			$('.oe_form_button_save').next().hide();
			$('.oe_form_button_save').next().next().hide();
		}
		/***** End Ticket 2984 */
			
		/***** Start Ticket 3051 */		
		$('*[class*="tab_move-"] input, *[class*="tab_move-"] textarea').on('keydown',function(e){
			var keyCode = e.keyCode || e.which; 
			var dir = '';			
			if(keyCode == 9) {			
				e.preventDefault(); 				
				dir = e.shiftKey?"bb":"ff";
				var class_str = $(this).closest("*[class*='tab_move-']").attr("class"),
				have_class = $(this).closest("*[class*='tab_move-']").attr("class").search("tab_move-");
				var got_class = "";				
				got_class = class_str.substr(have_class);
				split1 = got_class.split(" ");
				split2 = split1[0].split("-");

				var goTo = dir == "ff"?(parseInt(split2[1]) + 1):(split2[1] - 1);
				if(typeof($(".tab_move-"+goTo+" input")) != "undefined")
				$(".tab_move-"+goTo+" input").focus();
				$(".tab_move-"+goTo+" textarea").focus();
			}						
		});
		//Provide a class in the field as class="tab_move-1" class="tab_move-2" class="tab_move-3"
		/***** End Ticket 3051 */
    ...
    on_button_delete: function() {
        ...
        this.has_been_loaded.done(function() {
                swal({
                    title: "Are you sure?",
                    text: "Do you really want to remove these records?",
                    type: "warning",
                ...
                self.dataset.unlink([self.datarecord.id]).done(function() {
                    if (self.dataset.size()) {
                        self.execute_pager_action('next');
                                toastr.success("Your entry has been deleted!!", "Warning!");
                    } else {
                        self.do_action('history_back');
                    }
                    def.resolve();
                        }).fail(function (xhr, status, error) {
                            toastr.error(xhr.data.fault_code, "Warning!");
                });
    ...
    execute_action: function() {
        ...
        var exec_action = function() {
            if (self.node.attrs.confirm) {
                var def = $.Deferred();
				console.log(self.node.attrs.confirm)
				
				swal({
                title: "Confirm?",
                text: self.node.attrs.confirm,
                type: "warning",
                showCancelButton: true,
                confirmButtonColor: "#DD6B55",
...
instance.web.DateTimeWidget = instance.web.Widget.extend({
    ...
    start: function() {
		...
		/***** Start Ticket 3441 */
		var remove_readonly = $('.oe_datepicker_root').hasClass('remove_readonly');
		if(remove_readonly){
			$('span.oe_datepicker_root .oe_datepicker_master').attr('readonly',false);
		}
		/***** End Ticket 3441 */		
...
instance.web.form.FieldMany2One = instance.web.form.AbstractField.extend(instance.web.form.CompletionFieldMixin, instance.web.form.ReinitializeFieldMixin, {
    ...
    show_error_displayer: function() {
        new instance.web.form.M2ODialog(this).close(); // .open() to .close()
    },
...
instance.web.form.FieldMany2Many = instance.web.form.AbstractField.extend(instance.web.form.ReinitializeFieldMixin, {
    ...
    initialize_content: function() {
		...
		/* Edited w.r 1406 Add to Select*/
        this.list_view = new instance.web.form.Many2ManyListView(this, this.dataset, false, {
                    'addable': this.get("effective_readonly") ? null : _t("Select"),
