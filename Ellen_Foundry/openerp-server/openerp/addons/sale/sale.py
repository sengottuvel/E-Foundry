...
class sale_order(osv.osv):
	...
	_columns = {
		'name': fields.char('SO No', size=64,
			readonly=True, states={'draft': [('readonly', False)], 'sent': [('readonly', False)]}, select=True),
		'shop_id': fields.many2one('sale.shop', 'Shop', required=False, readonly=True, states={'draft': [('readonly', False)], 'sent': [('readonly', False)]},invisible=True),
		...
		'state': fields.selection([
			...
			('sent', 'Quotation Sent'),
			('cancel', 'Cancelled'),
			('waiting_date', 'Schedule Pending'),
			('progress', 'Schedule Completed'),
		...
		'company_id': fields.related('shop_id','company_id',type='many2one',relation='res.company',string='Company',store=True,readonly=True,invisible=True),
		'sale_type': fields.selection([('cash', 'Cash'), ('credit', 'Credit')], 'Sale Type'),
...
#~ 
	...
	def entry_confirm(self,cr,uid,ids,context=None):		
		...
		seq_id_sch = self.pool.get('ir.sequence').search(cr,uid,[('code','=','kg.schedule')])
		seq_rec_sch = self.pool.get('ir.sequence').browse(cr,uid,seq_id_sch[0])
		cr.execute("""select generatesequenceno(%s,'%s','%s') """%(seq_id_sch[0],seq_rec_sch.code,obj.date_order))
		seq_name_sch = cr.fetchone();		
		kg_schedule_id=self.pool.get('kg.schedule')
		ch_schedule_line_id=self.pool.get('ch.schedule.line')		
		if not obj.order_line:
			raise osv.except_osv(
						_('Warning'),
						_('You cannot allow to save with 0 order line'))
		if obj.type_of_delivery =='one_time':
			schedule_ids = kg_schedule_id.create(cr,uid,
					{
					'name':seq_name_sch[0],
					'customer_id':obj.partner_id.id,
					'customer_po':obj.customer_po,
					'so_no':obj.id,
					'customer_address':obj.customer_address,
					'customer_code':obj.customer_code,
					'so_date':obj.date_order,
					'state':'approved',
					'type_of_delivery':'one_time',
					})
					
			for i in obj.order_line:
				ch_schedule_line_id.create(cr,uid,
						{
				...
				cr.execute("""update sale_order_line set pending_qty=0 where order_id =%s"""%(obj.id))
	...
	def entry_approve(self,cr,uid,ids,context=None):	
		...
		for line in obj.order_line:
			...
			if line.product_id.type != 'service':	
				...
				if line.product_uom_qty > line.stock_qty:
					raise osv.except_osv(
						_('Warning'),
						_('Product Qty is greater than stock qty for : "%s"')%(line.product_id.name))
				...
				stock_move_obj.create(cr,uid,
								...
								'po_to_stock_qty':line.product_uom_qty,
								'stock_uom':line.product_uom.id,
								'product_uom': line.product_uom.id,
								'location_id': line.src_stock_location.id,
								'location_dest_id': line.des_stock_location.id,
								'move_type': 'cons',
								'state': 'done',
	...
	def daily_sales_register_scheduler(self,cr,uid,ids=0,context = None):
		cr.execute(""" SELECT current_database();""")
		db = cr.dictfetchall()
		
			
		cr.execute(""" select sum(amount_total) as total from sale_order where state='done' and sale_type in ('cash','credit')  and to_char(date_order,'yyyy-mm-dd') = '%s'  order by 1"""
						%(time.strftime('%Y-%m-%d')))
		sales_data = cr.dictfetchall()	
		print "---------------------------",sales_data	
		if sales_data[0]['total'] != None:
			print "lkdxhlkjsalkjdklhlkdsklshdlkhsalkdlksahdlkhsdasads"
			print "lkdxhlkjsalkjdklhlkdsklshdlkhsalkdlksahdlkhsdasads"
			print "lkdxhlkjsalkjdklhlkdsklshdlkhsalkdlksahdlkhsdasads"
			print "lkdxhlkjsalkjdklhlkdsklshdlkhsalkdlksahdlkhsdasads"
			print "lkdxhlkjsalkjdklhlkdsklshdlkhsalkdlksahdlkhsdasads"
			print "lkdxhlkjsalkjdklhlkdsklshdlkhsalkdlksahdlkhsdasads"
			print "lkdxhlkjsalkjdklhlkdsklshdlkhsalkdlksahdlkhsdasads"
			cr.execute("""select all_daily_scheduler_mails('Daily Sales Register')""")
			data = cr.fetchall();
			cr.execute("""select trim(TO_CHAR(sum(amount_total)::float, '999G999G99G999G99G99G990D99')) as total from sale_order where state='done' and sale_type in ('cash','credit')  and to_char(date_order,'dd-mm-yyyy') = '%s'  order by 1   """%(time.strftime('%d-%m-%Y')))
			total_sum = cr.dictfetchall();
			db = db[0]['current_database'].encode('utf-8')
			total_sum = str(total_sum[0]['total'])
			vals = self.sechedular_email_ids(cr,uid,ids,reg_string = 'daily sales register',context = context)
			...
			if (not vals['email_to']) and (not vals['email_cc']):
				...
				msg = ir_mail_server.build_email(
						...
						subject = "Stationery Store - Total Sales on "+time.strftime('%d-%m-%Y')+' is :' + total_sum,
						body = data[0][0],
						email_cc = vals['email_cc'],
						object_id = ids and ('%s-%s' % (ids, 'sale.order')),
						subtype = 'html',
	...
	def onchange_partner_id(self, cr, uid, ids, part, customer_code,context=None):
		if not part:
...
class sale_order_line(osv.osv):
	...
	def _get_scr_loc(self, cr, uid, line, context=None):
		
		location = self.pool.get('stock.location').search(cr, uid, [('name', '=', 'Sales location')])
		if location:
			return location[0]
		else:
			pass	
		
	def _get_des_loc(self, cr, uid, line, context=None):
		
		location = self.pool.get('stock.location').search(cr, uid, [('name', '=', 'KG_Consumption_Store')])
	...
	_columns = {
		...
		'name': fields.text('Name', required=False, readonly=True, states={'draft': [('readonly', False)]}),
		'sequence': fields.integer('Sequence', help="Gives the sequence order when displaying a list of sales order lines."),
		'product_id': fields.many2one('product.product', 'Product', domain=[('product_type', '=', 'finished_items'),('state','=','approved')], required=True,change_default=True),
		'invoice_lines': fields.many2many('account.invoice.line', 'sale_order_line_invoice_rel', 'order_line_id', 'invoice_id', 'Invoice Lines', readonly=True),
		'invoiced': fields.function(_fnct_line_invoiced, string='Invoiced', type='boolean',
			store={
				'account.invoice': (_order_lines_from_invoice, ['state'], 10),
				'sale.order.line': (lambda self,cr,uid,ids,ctx=None: ids, ['invoice_lines'], 10)}),
		'price_unit': fields.float('Unit Price', required=True, states={'draft': [('readonly', False)]}),
		...
		'product_uom_qty': fields.float('Accepted Qty', required=True, states={'draft': [('readonly', False)]}),
		'product_uom': fields.many2one('product.uom', 'Unit of Measure ', required=True, readonly=True, states={'draft': [('readonly', False)]}),
		'product_uos_qty': fields.float('Quantity (UoS)' ,digits_compute= dp.get_precision('Product UoS'), readonly=True, states={'draft': [('readonly', False)]}),
		'product_uos': fields.many2one('product.uom', 'Product UoS'),
		'discount': fields.float('Discount (%)', digits_compute= dp.get_precision('Discount'), readonly=True, states={'draft': [('readonly', False)]}),
		'th_weight': fields.float('Piece Weight(Kgs)', required=True, states={'draft': [('readonly', False)]}),
		...
		'src_stock_location': fields.many2one('stock.location', 'Source Location'),
		'des_stock_location': fields.many2one('stock.location', 'Destination Location'),
	...
	_defaults = {
		'product_uom' : False,
		'src_stock_location' : _get_scr_loc,
		'des_stock_location' : _get_des_loc,
		'discount': 0.0,
		'product_uom_qty': 1,
		'product_uos_qty': 1,
		'sequence': 10,
	...
	def _get_line_(self, cr, uid, line, context=None):
		if (line.order_id.invoice_quantity=='order'):
			if line.product_uos:
				return line.product_uos.id
		return line.product_uom.id
	...
	def uos_change(self, cr, uid, ids, product_uos, product_uos_qty=0, product_id=None):
		...
		print "product.uos_coeff,***************", product.uos_coeff
		try:
			value.update({
	...
	def create(self, cr, uid, vals, context=None):
		...
		loc = self.pool.get('stock.location').search(cr, uid, [('name', '=', 'KG_Consumption_Store')])
					
		out_sql = """ select product_id,sum(product_qty) as qty from stock_move where product_id=%s and move_type='out' and state='done' and location_dest_id = %s group by product_id """%(vals['product_id'],location[0])
		cr.execute(out_sql)			
		out_data = cr.dictfetchall()
		out_sql1 = """ select product_id,sum(product_qty) as qty from stock_move where product_id=%s and move_type='cons' and state='done' and location_dest_id = %s group by product_id """%(vals['product_id'],loc[0])
		cr.execute(out_sql1)			
		out_data1 = cr.dictfetchall()
	...
	def product_id_change(self, cr, uid, ids, pricelist, product, qty=0,
		...
		if uom:
			...
			print "uom 1 1 1", uom
			print "uom 2 2 2 2", uom2
			print "qtyyyyyyyyyyyyyyyyyyyy", qty
		...
		if (not uom) and (not uos):
			...
			print "qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq", q
			if product_obj.uos_id:
				result['product_uos'] = product_obj.uos_id.id
				result['product_uos_qty'] = qty * product_obj.uos_coeff
			else:
		...
		out_sql = """ select product_id,sum(product_qty) as qty from stock_move where product_id=%s and move_type='out' and state='done' and location_dest_id = %s group by product_id """%(product,location[0])
		cr.execute(out_sql)			
		out_data = cr.dictfetchall()
		
		out_sql1 = """ select product_id,sum(product_qty) as qty from stock_move where product_id=%s and move_type='cons' and state='done' and location_dest_id = %s group by product_id """%(product,loc[0])
		cr.execute(out_sql1)			
		out_data1 = cr.dictfetchall()
		...
		cr.execute("""select id,drawing_rev_number from kg_drawing_details where header_id ='%s' and draw_info='active'"""%(product))
		dr_detail = cr.dictfetchall()
		if dr_detail:
			if len(dr_detail) <=1:
				dr_no = dr_detail[0]['id']
				dr_rev_no = dr_detail[0]['drawing_rev_number']
			else:
				dr_no =''
				dr_rev_no =''
		else:
		...
		result['stock_qty'] = diff			
		result['product_uom'] = product_obj.uom_id.id
		result['product_uos'] = product_obj.uos_id.id			
		result['pending_qty'] = qty
		result['drawing_no'] =dr_no
		result['drawing_rev_no'] = dr_rev_no
