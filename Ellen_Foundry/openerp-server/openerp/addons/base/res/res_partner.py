...
class res_partner(osv.osv, format_address):
	...
	def _get_tz_offset(self, cr, uid, ids, name, args, context=None):
		result = dict.fromkeys(ids, False)
		for obj in self.browse(cr, uid, ids, context=context):
			result[obj.id] = datetime.now(pytz.timezone(obj.tz or 'GMT')).strftime('%z')
		return result
	...
	_columns = {
		...
		'kg_vendor_code':fields.integer('KG Vendor Code',size=128),
		'vendor_code':fields.integer('Vendor Code',size=128),
		'code':fields.char('Code',size=10),
		'vendor_pl_code':fields.integer('Vendor PL Code',size=128),
		'ad_code':fields.integer('AD Code',size=128),
		'segment': fields.selection([('domestic', 'Domestic'), ('export', 'Export'),
								 ('other', 'Other')], 'Segment'),
		'supplier_status':fields.selection([('approve', 'Approved'), ('not_approve', 'Waiting For Approval')], 'Supplier Status'),
		'sms_status':fields.selection([('applicable', 'Applicable'), ('not_applicable', 'Not Applicable')], 'SMS Status'),
		'email_status':fields.selection([('applicable', 'Applicable'), ('not_applicable', 'Not Applicable')], 'Email Status'),
		...
		'supplier_type': fields.selection([('all', 'All'), ('material', 'Material'),
								   ('service', 'Service')], 'Supplier Type'),
		...
		'sup_state': fields.selection([('draft','Draft'),('confirm','WFA'),('approved','Approved'),
				('reject','Rejected'),('cancel','Canceled')],'Status', readonly=True),
		'remark': fields.text('Remarks',readonly=False),
		#notes
		'notes': fields.text('Notes'),
		'birthdate': fields.datetime('Birth Date'),
	...
	def approve_supplier(self, cr, uid, ids, context=None):
		return self.write(cr, uid, ids, {'supplier_status':'approve'})
	...
	def _name_validate(self, cr, uid,ids, context=None):
		...
		if rec.name:
			...
			cr.execute(""" select upper(name) from res_partner where upper(name)  = '%s' """ %(name))
			data = cr.dictfetchall()
			if len(data) > 1:
				res = False
			else:
				res = True
#name validation for special characters not allowed
		for i in range(len(rec.name)):
			if re.match(r'[0-9!@#$%^~*{}?+=]+', rec.name[i]):
				raise osv.except_osv('Invalid Character', 'Special characters are not allowed in the Name field')
		return True
...
#check cst no
	...
	def _check_cst(self, cr, uid, ids, context=None):		
		rec = self.browse(cr, uid, ids[0])
		if rec.cst_no:
			if len(str(rec.cst_no)) >= 6 and len(str(rec.cst_no)) <= 11:
				return True
			else:
				return False
		return True	
...
#chec Job position
	...
	def _check_job_position(self, cr, uid, ids, context=None):
		rec = self.browse(cr, uid, ids[0])
		if rec.function != False:
			for i in range(len(rec.function)):
				if re.match(r'[0-9!@#$%^~*{}?+=]+', rec.function[i]):
					raise osv.except_osv('Invalid Character', 'Special characters are not allowed in the Job position field')
		return True			
	...
	_sql_constraints = [
		('adhar_id', 'unique(adhar_id)', 'Employee Adhar ID must be unique !!'),
		('name', 'unique(name)', 'The Name must be unique.Please enter a valid Name in Name field!!'),
		('code', 'unique(code)', 'The Code must be unique.Please enter a valid Code in Code field !!'),
	]

	def entry_confirm(self,cr,uid,ids,context=None):
		a = datetime.now()
		dt_time = a.strftime('%m/%d/%Y %H:%M:%S')
		self.write(cr, uid, ids, {'sup_state': 'confirm','conf_user_id': uid, 'confirm_date': dt_time})
		return True

	def entry_approve(self,cr,uid,ids,context=None):
		a = datetime.now()
		dt_time = a.strftime('%m/%d/%Y %H:%M:%S')
		self.write(cr, uid, ids, {'sup_state': 'approved','app_user_id': uid, 'approve_date':  dt_time})
		return True
	...
	def entry_reject(self,cr,uid,ids,context=None):
		...
		rec = self.browse(cr,uid,ids[0])
		if rec.remark:
			self.write(cr, uid, ids, {'sup_state': 'reject','rej_user_id': uid, 'reject_date':  dt_time})
		else:
