...
class kg_reshot_blasting(osv.osv):

	_name = "kg.reshot.blasting"
	_description = "Re-shot Blasting"
	...
	_columns = {
		...
		'name': fields.char('RB No' ,select=True,readonly=True),
		'entry_date':fields.date('RB Date',required=True),
		...
		'paint_id':fields.many2one('kg.painting.process','Painting  ID',readonly=True),		
	...
	_defaults = {
		...
		'company_id': lambda self,cr,uid,c: self.pool.get('res.company')._company_default_get(cr, uid, 'kg_reshot_blasting', context=c),			
	...
	def entry_confirm(self,cr,uid,ids,context=None):
		...
		if rec.state == 'draft':		
			if rec.name == '' or rec.name == False:
				seq_obj_id = self.pool.get('ir.sequence').search(cr,uid,[('code','=','kg.reshot.blasting')])
		...
		if rec.state == 'confirmed':	
			if rec.qty <=0:
				raise osv.except_osv(_('Warning!'),
						_('You can not allowed to save with 0 qty !!'))
	...
	def create(self, cr, uid, vals, context=None):
		return super(kg_reshot_blasting, self).create(cr, uid, vals, context=context)
		
		
	def write(self, cr, uid, ids, vals, context=None):
		vals.update({'update_date': time.strftime('%Y-%m-%d %H:%M:%S'),'update_user_id':uid})
		return super(kg_reshot_blasting, self).write(cr, uid, ids, vals, context)		
