...
class product_uom(osv.osv):
	...
	_columns = {
		...
		'ap_rej_date': fields.datetime('Rejected Date', readonly=True),
		'ap_rej_user_id': fields.many2one('res.users', 'Rejected By', readonly=True),			
	...
	_sql_constraints = [
		...
		('code', 'unique(code)', 'UOM Code must be unique!'),
	]

	def entry_confirm(self,cr,uid,ids,context=None):
		b = datetime.now()		
		d_time = b.strftime('%m/%d/%Y %H:%M:%S')
		self.write(cr, uid, ids, {'dummy_state': 'confirm','confirm_user_id': uid, 'confirm_date': d_time})
		return True

	def entry_approve(self,cr,uid,ids,context=None):
		b = datetime.now()		
		d_time = b.strftime('%m/%d/%Y %H:%M:%S')
		self.write(cr, uid, ids, {'dummy_state': 'approved','app_user_id': uid, 'approve_date': d_time})
		return True
	...
	def entry_reject(self,cr,uid,ids,context=None):
		...
		if rec.remark:
			self.write(cr, uid, ids, {'dummy_state': 'reject','ap_rej_user_id': uid, 'ap_rej_date': d_time})
		else:
	...
	def entry_cancel(self,cr,uid,ids,context=None):
		...
		if rec.cancel_remark:
			self.write(cr, uid, ids, {'dummy_state': 'cancel','cancel_user_id': uid, 'cancel_date': d_time})
		else:
	...
	def _compute_price(self, cr, uid, from_uom_id, price, to_uom_id=False):
		...
		if to_uom_id:
			amount = amount or 1 / to_unit.factor or 1
		return amount
...
class product_template(osv.osv):
	...
	_columns = {
		...
		'categ_id': fields.many2one('product.category','Category', change_default=True, domain="[('type','=','normal'),('state','=','approved')]" ,help="Select category for the current product"),
		...
		'uom_id': fields.many2one('product.uom', 'Store UOM', domain="[('dummy_state','=','approved')]" ,required=True, help="Default Unit of Measure used for all stock operation."),
		'uom_po_id': fields.many2one('product.uom', 'PO UOM', domain="[('dummy_state','=','approved')]" ,required=True, help="Default Unit of Measure used for purchase orders. It must be in the same category than the default unit of measure."),
...
class product_product(osv.osv):
	...
	_columns = {
		...
		'product_code':fields.char('Code', size=10),
		...
		'minimum_qty':fields.float('Minimum Stock Qty'),
	    ...
	    'inhouse_rejection':fields.selection([('yes', 'Yes'), ('no', 'No')], 'Inhouse Rejection'),
	    'rack_details':fields.char('Rack Details'),
	...
	_sql_constraints = [
		...
		('product_code', 'unique(product_code)', 'Product code must be unique!'),
	]
