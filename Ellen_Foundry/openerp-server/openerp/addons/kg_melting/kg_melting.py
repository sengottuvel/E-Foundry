...
class kg_melting(osv.osv):
	...
	_columns = {
		...
		'name': fields.char('Melting No' ,select=True,readonly=True),
		'entry_date':fields.date('Melting Date',required=True),
		...
		'mould_id':fields.many2one('kg.moulding','Moulding No',readonly=True),
		'shift':fields.char('Shift'),
		'mould_date':fields.date('Moulding Date',readonly=True),
		'employee_id':fields.many2one('hr.employee','Shift Supervisor',domain=[('id','!=',1),('state','=','approved'),('active','=',True)]),
		
		## Child Tables Declaration
				
		'line_ids':fields.one2many('ch.melting.line', 'header_id', 'Line Details'),
	...
	def entry_confirm(self,cr,uid,ids,context=None):
		...
		if rec.state == 'draft':		
			if rec.name == '' or rec.name == False:
				seq_obj_id = self.pool.get('ir.sequence').search(cr,uid,[('code','=','kg.melting')])
			...
			for i in rec.line_ids:
				...
				if not i.melting_log_id:
					raise osv.except_osv(
							_('Warning'),
							_('Please select the Furnance Heat No'))
	...
	def entry_approve(self,cr,uid,ids,context=None):
		...
		rec = self.browse(cr,uid,ids[0])
		fis_inspec_id = self.pool.get('kg.first.inspection')
		fis_inspec_line_id = self.pool.get('ch.first.inspection.line')		
			...
			for i in rec.line_ids:
				...
				if not i.melting_log_id:
					raise osv.except_osv(
							_('Warning'),
							_('Please select the Furnance Heat No'))			
			inspec_rec = fis_inspec_id.create(cr, uid, {'melt_id':rec.id,'entry_mode':'auto','state':'draft',})	
			for i in rec.line_ids:
				fis_inspec_line_id.create(cr, uid, {
	...
	def create(self, cr, uid, vals, context=None):
		return super(kg_melting, self).create(cr, uid, vals, context=context)
...
class ch_melting_line(osv.osv):

	_name = "ch.melting.line"
	_description = "Melting Line"
	
	_columns = {
	
		## Basic Info
		
		'header_id':fields.many2one('kg.melting', 'Melting', required=1, ondelete='cascade'),
		'remark': fields.text('Remarks'),
		'active': fields.boolean('Active'),			
		...
		'pouring_weight':fields.float('Pouring Wgt'),
		'tot_box':fields.float('No.of Box'),
		'sl':fields.float('SL'),
		'cm':fields.float('CM'),
		'shr':fields.float('SHR'),
		'shp':fields.float('SHP'),
		'leak':fields.float('Leak'),
		'hdn':fields.float('HDN'),
