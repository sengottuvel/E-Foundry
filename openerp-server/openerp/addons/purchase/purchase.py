...
class purchase_order(osv.osv):
	...
	_columns = {
		...
		'partner_id':fields.many2one('res.partner', 'Supplier', required=True, states={'approved':[('readonly',True)],'done':[('readonly',True)],'cancel':[('readonly',True)]}, domain=[('supplier','=',True),('sup_state','=','approved')],
...
class purchase_order_line(osv.osv):
	...
	def onchange_product_id(self, cr, uid, ids, pricelist_id, product_id, qty, uom_id,
		...
		if partner_id:
			...
			raise osv.except_osv(_(' Warning!!'),_("Please select the supplier name!") )
